    ...
    <div class="card">
      <div class="settings-layout">
        ...
        <nav class="settings-nav">
          ...
          <div v-for="item in navItems" :key="item.key" class="snav-item"
          ...
          <div v-for="item in navPrefs" :key="item.key" class="snav-item"
            :class="{ active: activeSection === item.key }" @click="activeSection = item.key">
            <i :class="`ti ${item.icon}`" aria-hidden="true"></i>
            {{ item.label }}
          </div>
          <div class="snav-section">Suporte</div>
          <div v-for="item in navSupport" :key="item.key" class="snav-item"
            :class="{ active: activeSection === item.key, danger: item.danger }"
            @click="item.danger ? handleLogout() : (activeSection = item.key)">
            <i :class="`ti ${item.icon}`" aria-hidden="true"></i>
            {{ item.label }}
        ...
        <div class="settings-content">
          ...
          <div>
            ...
            <div class="fields-col">
              <div class="fields-grid">
                <SettingsField id="s-nome" label="Nome" placeholder="João" icon="ti-user" v-model="form.nome" />
                <SettingsField id="s-sobrenome" label="Sobrenome" placeholder="Silva" v-model="form.sobrenome" />
              </div>
              <SettingsField id="s-handle" label="Usuário" placeholder="seuusuario" icon="ti-at" v-model="form.handle"
                :hint="`nova.app/@${form.handle}`" />
              <SettingsField id="s-bio" label="Bio" placeholder="Conte um pouco sobre você..." v-model="form.bio"
                :multiline="true" :rows="3" :maxlength="160" :hint="`${form.bio.length} / 160 caracteres`" />
            </div>
          </div>
          ...
          <div>
            ...
            <SettingsField id="s-email" label="E-mail" type="email" placeholder="voce@exemplo.com" icon="ti-mail"
              v-model="form.email" />
          </div>
          ...
          <div>
            ...
            <div class="fields-col">
              <SettingsField id="s-pass-cur" label="Senha atual" type="password" placeholder="••••••••" icon="ti-lock"
                v-model="form.currentPass" />
              <div class="fields-grid">
                <SettingsField id="s-pass-new" label="Nova senha" type="password" placeholder="••••••••"
                  icon="ti-lock-plus" v-model="form.newPass" />
                <SettingsField id="s-pass-con" label="Confirmar nova senha" type="password" placeholder="••••••••"
                  icon="ti-lock-check" v-model="form.confirmPass" />
              </div>
          ...
          <div>
            ...
            <div class="danger-zone">
              <div class="danger-info">
                ...
                <p class="danger-sub">Remove permanentemente sua conta e todos os seus dados. Esta ação não pode ser
                  desfeita.</p>
              </div>
...
async function handleAvatarChange(e) {
  ...
  try {
    const userId = settings.user?.id || settings.user

    await settings.addImage(file, userId)
  } catch (err) {
    console.error(err)
  }
}
...
.card {
  ...
  .settings-content {
    padding: 32px 48px;
    /* padding lateral generoso para não "esgassar" */
    display: flex;
    flex-direction: column;
    gap: 28px;
  }
  ...
  .section-divider {
    ...
    margin: 0 -48px;
    /* compensa o padding lateral do .settings-content */
  }
  ...
  /* Responsivo: em telas menores, volta a ter padding e a navegação vira horizontal */
